...
@RabbitListener(
		bindings = @QueueBinding(
				value = @Queue(value = RabbitmqConf.UNDO_SYNC_PATH_QUEUE, durable = "true"),
				exchange = @Exchange(value = RabbitmqConf.UNDO_SYNC_PATH_EXCHANGE, ignoreDeclarationExceptions = "true"),
				key = RabbitmqConf.UNDO_SYNC_PATH_ROUTEKEY)
		)
public class RabbitmqListener_UNDO_SYNC_PATH_QUEUE {
	
	private static final Logger logger = LoggerFactory.getLogger(RabbitmqListener_UNDO_SYNC_PATH_QUEUE.class);  
    ...
    public void process(
    	...
    	try {
			...
			Map<String, Object> retMap = (Map<String, Object>) ftpService.handlePath(ajbs, ajlx, fydm, uuid);
			if(retMap.get(CodeUtil.RETURN_CODE).toString().equalsIgnoreCase(CodeUtil.RETURN_SUCCESS)){
				...
				if(redisTemplate.hasKey(key)) {
					...
					logger.info("处理路径信息异常，"+retMap.get(CodeUtil.RETURN_MSG).toString()+";全局锁已清除："+rabbitmqMessage.get(Constant.AJBS).toString());
					loggerService.logger(
							...
							fydm, 
							"003", 
							"处理路径信息异常，"+retMap.get(CodeUtil.RETURN_MSG).toString()+";全局锁已清除："+rabbitmqMessage.get(Constant.AJBS).toString());
				}else {
					logger.info("处理路径信息异常，"+retMap.get(CodeUtil.RETURN_MSG).toString()+";全局锁不存在："+rabbitmqMessage.get(Constant.AJBS).toString());
					loggerService.logger(
							applicationName, 
							10, 
							uuid, 
							...
							fydm, 
							"003", 
							"处理路径信息异常，"+retMap.get(CodeUtil.RETURN_MSG).toString()+";全局锁不存在："+rabbitmqMessage.get(Constant.AJBS).toString());
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
