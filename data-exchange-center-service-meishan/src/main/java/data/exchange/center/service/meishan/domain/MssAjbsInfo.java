package data.exchange.center.service.meishan.domain;

import java.io.Serializable;

/**
 * 
 * Description:民商事结构化数据
 * <p>Company: xinya </p>
 * <p>Date:2017年9月13日 上午11:30:39</p>
 * @author Wen.Yuguang
 * @version 1.0
 *
 */
public class MssAjbsInfo implements Serializable {

	/**
	 * 2017年9月13日上午11:30:34
	 * yuguang
	 */
	private static final long serialVersionUID = 1620661145744894404L;
	
	private String AJLX;
	private String SPZ;
	private String SFSQTJ;
	private String JAFS;
	private String YY;
	private String YCQJ;
	private String KTSL;
	private String AJJZJD;
	private String SYCX;
	private String LARQ;
	private String AH;
	private String LAAY; 
	private String BDJE;
	private String CSCLSY; 
	private String AY;
	private String JARQ;
	private String JABDJE;
	private String FDSXTS;
	private String JYZPTRQ;
	private String TC;
	private String SJSLTS;
	private String STFY;
	private String JAAY;
	private String FGZL;
	private String GPYY;
	private String LADM;
	private String JADM;
	private String SJDM;
	private String FJM;
	public String getAJLX() {
		return AJLX;
	}
	public void setAJLX(String aJLX) {
		AJLX = aJLX;
	}
	public String getSPZ() {
		return SPZ;
	}
	public void setSPZ(String sPZ) {
		SPZ = sPZ;
	}
	public String getSFSQTJ() {
		return SFSQTJ;
	}
	public void setSFSQTJ(String sFSQTJ) {
		SFSQTJ = sFSQTJ;
	}
	public String getJAFS() {
		return JAFS;
	}
	public void setJAFS(String jAFS) {
		JAFS = jAFS;
	}
	public String getYY() {
		return YY;
	}
	public void setYY(String yY) {
		YY = yY;
	}
	public String getYCQJ() {
		return YCQJ;
	}
	public void setYCQJ(String yCQJ) {
		YCQJ = yCQJ;
	}
	public String getKTSL() {
		return KTSL;
	}
	public void setKTSL(String kTSL) {
		KTSL = kTSL;
	}
	public String getAJJZJD() {
		return AJJZJD;
	}
	public void setAJJZJD(String aJJZJD) {
		AJJZJD = aJJZJD;
	}
	public String getSYCX() {
		return SYCX;
	}
	public void setSYCX(String sYCX) {
		SYCX = sYCX;
	}
	public String getLARQ() {
		return LARQ;
	}
	public void setLARQ(String lARQ) {
		LARQ = lARQ;
	}
	public String getAH() {
		return AH;
	}
	public void setAH(String aH) {
		AH = aH;
	}
	public String getLAAY() {
		return LAAY;
	}
	public void setLAAY(String lAAY) {
		LAAY = lAAY;
	}
	public String getBDJE() {
		return BDJE;
	}
	public void setBDJE(String bDJE) {
		BDJE = bDJE;
	}
	public String getCSCLSY() {
		return CSCLSY;
	}
	public void setCSCLSY(String cSCLSY) {
		CSCLSY = cSCLSY;
	}
	public String getAY() {
		return AY;
	}
	public void setAY(String aY) {
		AY = aY;
	}
	public String getJARQ() {
		return JARQ;
	}
	public void setJARQ(String jARQ) {
		JARQ = jARQ;
	}
	public String getJABDJE() {
		return JABDJE;
	}
	public void setJABDJE(String jABDJE) {
		JABDJE = jABDJE;
	}
	public String getFDSXTS() {
		return FDSXTS;
	}
	public void setFDSXTS(String fDSXTS) {
		FDSXTS = fDSXTS;
	}
	public String getJYZPTRQ() {
		return JYZPTRQ;
	}
	public void setJYZPTRQ(String jYZPTRQ) {
		JYZPTRQ = jYZPTRQ;
	}
	public String getTC() {
		return TC;
	}
	public void setTC(String tC) {
		TC = tC;
	}
	public String getSJSLTS() {
		return SJSLTS;
	}
	public void setSJSLTS(String sJSLTS) {
		SJSLTS = sJSLTS;
	}
	public String getSTFY() {
		return STFY;
	}
	public void setSTFY(String sTFY) {
		STFY = sTFY;
	}
	public String getJAAY() {
		return JAAY;
	}
	public void setJAAY(String jAAY) {
		JAAY = jAAY;
	}
	public String getFGZL() {
		return FGZL;
	}
	public void setFGZL(String fGZL) {
		FGZL = fGZL;
	}
	public String getGPYY() {
		return GPYY;
	}
	public void setGPYY(String gPYY) {
		GPYY = gPYY;
	}
	public String getLADM() {
		return LADM;
	}
	public void setLADM(String lADM) {
		LADM = lADM;
	}
	public String getJADM() {
		return JADM;
	}
	public void setJADM(String jADM) {
		JADM = jADM;
	}
	public String getSJDM() {
		return SJDM;
	}
	public void setSJDM(String sJDM) {
		SJDM = sJDM;
	}
	public String getFJM() {
		return FJM;
	}
	public void setFJM(String fJM) {
		FJM = fJM;
	}
	@Override
	public String toString() {
		return "MssAjbsInfo [AJLX=" + AJLX + ", SPZ=" + SPZ + ", SFSQTJ=" + SFSQTJ + ", JAFS=" + JAFS + ", YY=" + YY
				+ ", YCQJ=" + YCQJ + ", KTSL=" + KTSL + ", AJJZJD=" + AJJZJD + ", SYCX=" + SYCX + ", LARQ=" + LARQ
				+ ", AH=" + AH + ", LAAY=" + LAAY + ", BDJE=" + BDJE + ", CSCLSY=" + CSCLSY + ", AY=" + AY + ", JARQ="
				+ JARQ + ", JABDJE=" + JABDJE + ", FDSXTS=" + FDSXTS + ", JYZPTRQ=" + JYZPTRQ + ", TC=" + TC
				+ ", SJSLTS=" + SJSLTS + ", STFY=" + STFY + ", JAAY=" + JAAY + ", FGZL=" + FGZL + ", GPYY=" + GPYY
				+ ", LADM=" + LADM + ", JADM=" + JADM + ", SJDM=" + SJDM + ", FJM=" + FJM + "]";
	}
}
