...
public class CommUtils {
	...
	/**
	 * 下载xml文件到临时文件夹
	 * （此方法只能用户HTTP协议。获取文件成功后可根据"File file = new File(filename);"获得该文件file）
	 * @param url  http路径URL
	 * @param filename  临时存放文件名（含临时文件夹路径）
	...
	/**
	 * 除去字符串中非以下情形的字符（慎用：含全角字符的请转半角后再使用此方法）：
	 * <BR>1、中文、英文、数字
	 * <BR>2、英文符号+*-/,.;:'"()_[]
	 * <BR>3、中文符号【】、，。·——
	 * @param src
	...
	public static String decodeLowHex(String base64Str){
		if(isEmptyStr(base64Str)) return "";
		byte[] data =  null;
		try {
	...
	public static String formatDateTime(String text,String stype,String dtype){
		...
		if(stype.indexOf("-")>-1 && text.indexOf("-")==-1) return text;
		if(stype.indexOf("-")==-1 && text.indexOf("-")>-1) return text;
	...
	public static  String getAjlx(String ajlx){
		if(ajlx.equals("11")){
			return "XSYS";
